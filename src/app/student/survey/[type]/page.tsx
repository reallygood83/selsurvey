...
// 일일 설문 질문 (저학년용 이모지 기반) - SEL 템플릿 ID 매핑
const dailyQuestionsLower = [
  {
    id: 'sa2', // 자기인식 - 현재 감정 상태
    question: '지금 기분이 어때요?',
    type: 'emoji_scale',
    options: ['😢', '😟', '😐', '😊', '😄'],
  ...
  },
  {
    ...
    domain: 'relationshipSkills'
  }
];

// 일일 설문 질문 (고학년용 텍스트 기반) - SEL 템플릿 ID 매핑
const dailyQuestionsUpper = [
  ...
  {
    id: 'rdm1', // 책임감 있는 의사결정 - 만족도/자기평가
    question: '오늘 하루에 얼마나 만족하나요?',
    type: 'scale',
    min: 1,
    max: 5,
    labels: ['매우 불만족', '불만족', '보통', '만족', '매우 만족'],
    domain: 'responsibleDecisionMaking'
  }
];

// 주간 설문 질문 - SEL 템플릿 ID 매핑
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const responsesArray = Object.entries(responses).map(([questionId, answer]) => {
        // 질문 ID로 실제 질문 객체를 찾아서 domain 가져오기
        const question = questions.find(q => q.id === questionId);
        return {
          questionId,
          answer,
          domain: question?.domain || 'selfAwareness' as const // 질문의 실제 domain 사용
        };
      });
